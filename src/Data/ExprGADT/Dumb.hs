...
unDumbWith env e = do
        ...
        case o of
            ...
            case t of
              TEO0 TOInt ->
                return $ PE n (\ts -> case f ts of
                                        EW vs EInt e1' -> EW vs EInt (O1 Abs e1')
                                        EW{} -> error "bug in inferExpr"
                              )
              TEV v -> throwError TErrBottom
              t     -> throwError $ TErrMismatch [t] [TEO0 TOInt]
          Signum ->
            case t of
              TEO0 TOInt ->
                return $ PE n (\ts -> case f ts of
                                        EW vs EInt e1' -> EW vs EInt (O1 Signum e1')
                                        EW{} -> error "bug in inferExpr"
                              )
              TEV v -> throwError TErrBottom
              t     -> throwError $ TErrMismatch [t] [TEO0 TOInt]
          Not ->
            case t of
              TEO0 TOBool ->
                return $ PE n (\ts -> case f ts of
                                        EW vs EBool e1' -> EW vs EBool (O1 Not e1')
                                        EW{} -> error "bug in inferExpr"
                              )
              TEV v -> throwError TErrBottom
              t     -> throwError $ TErrMismatch [t] [TEO0 TOBool]
          Left' ->
            return $ PE (SNS n) (\(ETW t :> ts) ->
                                    case f ts of
                                      EW vs t' e1' -> EW vs (EEither t' t) (O1 Left' e1')
                                )
          Right' ->
            return $ PE (SNS n) (\(ETW t :> ts) ->
                                    case f ts of
                                      EW vs t' e1' -> EW vs (EEither t t') (O1 Right' e1')
                                )
          Fst ->
            case t of
              TEO2 TOTuple t1 t2 ->
                return $ PE n (\ts -> case f ts of
                                        EW vs (ETup t1' t2') e1' -> EW vs t1' (O1 Fst e1')
                                        EW{} -> error "bug in inferExpr"
                              )
              TEV v -> throwError TErrBottom
          ...
          Snd ->
            case t of
              TEO2 TOTuple t1 t2 ->
                return $ PE n (\ts -> case f ts of
                                        EW vs (ETup t1' t2') e1' -> EW vs t2' (O1 Snd e1')
                                        EW{} -> error "bug in inferExpr"
                              )
              TEV v -> throwError TErrBottom
              t     -> throwError $ TErrMismatch [t] [TEO2 TOTuple (TEV (TV "a")) (TEV (TV "b"))]
  ...
  where
    ...
    -- Forall vs t <- inferExpr env e
    -- case t of
    --   TEO0 TOInt  -> undefined
    --   TEO0 TOBool -> undefined
    --   TEO0 TOUnit -> case e of
    --                    DO0 Unit -> return . PE SNZ $ \_ -> EW ENil EUnit (O0 Unit)
...
--     nSub NZ     y = y
--     nSub (NS x) NZ = NZ
--     nSub (NS x) (NS y) = nSub x y


--     nComp :: PNat -> PNat -> Ordering
--     nComp NZ     NZ     = EQ
--     nComp (NS _) NZ     = GT
--     nComp NZ     (NS _) = LT
--     nComp (NS x) (NS y) = nComp x y

--     nLTE :: PNat -> PNat -> Bool
--     nLTE NZ y = True
--     nLTE (NS x) (NS (NS y)) = nLTE x (NS y)
...
-- deriving instance Show (NatIxor n)
...
--     -- TESub    :: TExpr b ('NS u) -> NatIxor ('NS u) -> TExpr' u -> TExpr b u
...
--              -- TESub e1 i t -> let fu' :: NatIxor ('NS u) -> a
--              --                     fu' j = fromTExpr undefined undefined f0 f1 f2
--              --                           $ case compare j i of
--              --                               EQ -> t
--              --                               LT -> undefined
--              --                 in  fromTExpr fb fu' f0 f1 f2 e1

-- -- popOut :: NatIxor ('NS n) -> NatIxor ('NS n) -> NatIxor n
-- -- popOut i j = case nixToSS j of
-- --                SomeSing n -> case dropOut n j of
-- --                                Nothing -> j
-- --                                Just k -> _

-- -- nixToSS :: NatIxor n -> SomeSing ('KProxy :: KProxy PNat)
-- -- nixToSS NIZ = SomeSing SNZ
-- -- nixToSS (NIS i) = case nixToSS i of
-- --                     SomeSing n -> SomeSing (SNS n)

-- -- dropOut :: Sing n -> NatIxor m -> Maybe (NatIxor (NSub n m))
-- -- dropOut SNZ i = Just i
-- -- dropOut (SNS _) NIZ = Nothing
-- -- dropOut (SNS n) (NIS i) = dropOut n i


-- withVars :: (NatIxor b -> TExpr b' u')
--          -> (NatIxor u -> TExpr b' u')
--          -> TExpr b u
--          -> TExpr b' u'
-- withVars fb fu = fromTExpr fb fu TEO0 TEO1 TEO2

-- transplant :: TExpr b u' -> TExpr b u -> Maybe (TExpr b u)
-- transplant (TEVU u) t = Just t
-- transplant (TEVB b) _ = Just $ TEVB b
-- transplant (TEO0 o) _ = Just $ TEO0 o
-- transplant (TEO1 o t1) (TEO1 o' t1') | o == o' = TEO1 o <$> transplant t1 t1'
-- transplant (TEO2 o t1 t2) (TEO2 o' t1' t2') | o == o' = TEO2 o <$> transplant t1 t1' <*> transplant t2 t2'
-- transplant (TEForall t1) (TEForall t1') = TEForall <$> transplant t1 t1'
-- transplant _ _ = Nothing
...
-- data TExprUW :: PNat -> * where
--     TEUW :: Sing u -> TExpr b u -> TExprUW b

-- type TExprUW' = TExprUW NZ
...
-- instantiate :: forall b u. ReNix b (NPlus b u) => TExpr b u -> TExpr 'NZ (NPlus b u)
-- instantiate = withVars (TEVU . reNix) undefined

-- class ReNix (n :: PNat) (m :: PNat) where
--     reNix :: NatIxor n -> NatIxor m

-- instance ReNix 'NZ m where
--     reNix = impossible ""
...
--   where
--     h :: NatIxor u -> TExpr' w
--     h = withVars (impossible "") g . f

-- composeSubst' :: forall u (q :: PNat). SubstW u -> SubstW u -> SubstW u
-- composeSubst' (SW (n1 :: Sing v) (Subst f1))
--               (SW (n2 :: Sing w) (Subst f2))
--             = SW n2 (Subst f3)
--   where
--     -- n3 :: Sing q
--     -- n3 = undefined
--     f3 :: NatIxor u -> TExpr' w
--     f3 i = let t1 = f1 i
--                t2 = f2 i
--            in  fromMaybe (error "structural mismatch") $ transplant t1 t2
...
--     SW :: Sing v -> Subst u v -> SubstW u

-- unify :: SingI u => TExpr' u -> TExpr' u -> Either TypeError (SubstW u)
-- unify (TEO0 o) (TEO0 o') | o == o' = return $ SW sing nullSubst
-- unify (TEO1 o t) (TEO1 o' t') | o == o' = unify t t'
-- unify (TEO2 o t s) (TEO2 o' t' s') | o == o' = do
--     sw1 <- unify t t'
--     sw2 <- unify s s'
--     return $ composeSubst' sw1 sw2
-- unify (TEVU v) t = bind v t
-- unify t (TEVU v) = bind v t
-- unify (TEVB _) (TEVB _) = impossible ""
-- unify _ _ = throwError TErrUniFail

-- bind :: forall u. SingI u => NatIxor u -> TExpr' u -> Either TypeError (SubstW u)
-- bind v t = case t of
--              TEVU v' | v == v'       -> return $ SW sing nullSubst
...
-- -- -- too lazy atm.
-- -- singleSubst :: NatIxor ('NS u) -> TExpr' u -> Subst ('NS u) u
-- -- singleSubst v t = Subst $ \i -> if v == i then t
-- --                                           else case i of
-- --                                                  NIZ -> TEVU NIZ

-- infer :: forall u. Env u -> DumbExpr -> Either TypeError (Subst u u, TExprUW')
-- infer env@(Env mp) e =
--     case e of
--       DV v -> case M.lookup v mp of
--                 Nothing -> throwError $ TErrUnboundVar v
--                 Just (TEBW b t)  -> return (nullSubst, TEUW undefined undefined)
--       DLambda v e1 -> do
